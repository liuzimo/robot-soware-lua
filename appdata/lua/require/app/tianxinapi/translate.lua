...
return function (msg)
    ...
    for i=1,l do
        ...
        if asc2>127 then
            local html = apiHttpPost("http://api.tianapi.com/txapi/fanyi/","key="..tianxinkey.."&text="..key.."&to=en")
    ...
    local html = apiHttpPost("http://api.tianapi.com/txapi/fanyi/","key="..tianxinkey.."&text="..key.."&to=zh")
    if not html then return "网络错误" end
    local j= jsonDecode(html)
    if j["msg"] ~="success" then return "检查api调用次数是否足够" end
    return j["newslist"][1]["dst"]
